    ...
    for (NSExtensionItem *item in self.extensionContext.inputItems) {
        for (NSItemProvider *itemProvider in item.attachments) {
            if ([itemProvider hasItemConformingToTypeIdentifier:(NSString *)kUTTypeLivePhoto]) {
                ...
                // use id<NSSecureCoding, NSObject> instead of NSURL because of rdar://29924023
                [itemProvider loadItemForTypeIdentifier:(NSString *)kUTTypeLivePhoto options:nil completionHandler:^(id<NSSecureCoding, NSObject> item, NSError *error) {
                ...
                [[NSOperationQueue mainQueue] addOperationWithBlock:^{
                    UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"Arbitrary image shared" message:@"An arbitrary image was shared." preferredStyle:UIAlertControllerStyleAlert];
                    [alert addAction:[UIAlertAction actionWithTitle:@"OK" style:UIAlertActionStyleCancel handler:NULL]];
                    [self presentViewController:alert animated:YES completion:NULL];
                }];
