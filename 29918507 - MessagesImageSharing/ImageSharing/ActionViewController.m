    ...
    for (NSExtensionItem *item in self.extensionContext.inputItems) {
        for (NSItemProvider *itemProvider in item.attachments) {
            if ([itemProvider hasItemConformingToTypeIdentifier:(NSString *)kUTTypeFileURL]) {
                ...
                [itemProvider loadItemForTypeIdentifier:(NSString *)kUTTypeFileURL options:nil completionHandler:^(NSURL *fileURL, NSError *error) {
                    ...
                    NSError *copyError;
                    if ([fileManager copyItemAtURL:fileURL toURL:targetURL error:&copyError]) {
                        [[NSOperationQueue mainQueue] addOperationWithBlock:^{
                            [imageView setImage:[UIImage imageWithContentsOfFile:targetURL.path]];
                        }];
                    } else {
                        [[NSOperationQueue mainQueue] addOperationWithBlock:^{
                            UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"Error during -[NSFileManager copyItemAtURL:toURL:error:]" message:copyError.localizedDescription preferredStyle:UIAlertControllerStyleAlert];
